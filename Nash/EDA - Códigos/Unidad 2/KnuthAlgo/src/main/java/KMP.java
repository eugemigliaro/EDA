public class KMP {
	...
	public static int indexOf( char[] query, char[] target) {
		...
		int[] borders = nextComputation2(query);

		int rec = 0;		//Para recorrer target
		int pquery = 0;		//Para recorrer query

		while(pquery < query.length && rec < target.length){
